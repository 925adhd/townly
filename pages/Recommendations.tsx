...
const Recommendations: React.FC<RecommendationsProps> = ({ requests, setRequests, user }) => {
  ...
  const handleShare = async (req: RecommendationRequest) => {
    const url = req.slug ? `${window.location.origin}/ask/${req.slug}` : `${window.location.origin}/ask`;
    if (navigator.share) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col items-center text-center gap-4">
        <div className="space-y-1.5">
          ...
          <p className="text-slate-400 text-xs mt-2">Try the <Link to="/directory" className="text-blue-400 hover:underline">Business Directory</Link> first. Posting an event or announcement? Use <Link to="/events" className="text-blue-400 hover:underline">Events & Announcements</Link>.</p>
        </div>
        {!showForm && (
          <div className="pt-1">
            {user ? (
